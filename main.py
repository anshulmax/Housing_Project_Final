...
# combine databases on guid
HousingAndIncome = housingDataFrame.join(incomeDataFrame.set_index('guid'),
                                         on='guid',
                                         lsuffix='_housing',
                                         rsuffix='_income')
Combined_Data_Frame = HousingAndIncome.join(zipDataFrame.set_index('guid'),
                                            on='guid')
Combined_Data_Frame = Combined_Data_Frame.drop(columns=['zip_code_housing', 'zip_code_income'])
Combined_Data_Frame = Combined_Data_Frame.rename(columns={'housing_median_age': 'median_age'})
# remove hyphens to allow it to be the right size for it to work.
for guid, row in Combined_Data_Frame.iterrows():
    stripped = row['guid'].replace('-', "")
    Guid_List.append(stripped)
GuidDf = pd.DataFrame(data=Guid_List, columns=['guid'])
Combined_Data_Frame.update(GuidDf)
# cleaning data
...
for guid, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptGuid, np.nan, True)
        Combined_Data_Frame.dropna(inplace=True)
    else:
        pass
# assigning corrupt zip code with the beginning number followed by 0000 of good zip code of the same state
for code, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        City = row['city']
        State = row['state']
        for CityState, record in Combined_Data_Frame.iterrows():
            valid_check = re.search("[A-Za-z][A-Za-z][A-Za-z][A-Za-z]", record['zip_code'])
...
for rooms, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptRooms, randRoom, True)
    else:
        pass
for bedrooms, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptBedroom, randBedroom, True)
    else:
        pass
for population, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptPopulation, randPopulation, True)
    else:
        pass
for households, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptHouseholds, randHouseholds, True)
    else:
        pass
for value, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptValue, randValue, True)
    else:
        pass
for income, row in Combined_Data_Frame.iterrows():
    ...
    if check is not None:
        ...
        Combined_Data_Frame.replace(CorruptIncome, randIncome, True)
    else:
        pass
...
ColumnList = "`,`".join([str(i) for i in Combined_Data_Frame.columns.tolist()])
for i, row in Combined_Data_Frame.iterrows():
